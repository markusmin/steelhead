...
hatchery, stock name, mark capture method, 
...
- 29 Mark Length: Less than or equal to 350 mm (to ensure you are only keeping individuals tagged as Juveniles)
...
# Load table from Susannah on site info
...
adult_returns %>% 
  ...
  dplyr::select(tag_code, first_time_value, rear_type_code, release_site_name, release_site_basin_name, release_site_subbasin_name,
                mark_site_basin_name, mark_site_subbasin_name, mark_site_name, hatchery_name, stock_name) %>% 
...
table(subset(adult_returns_subset, mark_site_subbasin_name == "Lower Snake-Tucannon" & rear_type_code == "H")$mark_site_name) # mark sites, hatchery only
...
lwrsnk_tuc_release_fish <- subset(adult_returns_subset, release_site_subbasin_name == "Lower Snake-Tucannon")
table(adult_returns_subset$release_site_subbasin_name)

# Export this as tag codes to use to query PTAGIS
write.table(as.data.frame(unique(lwrsnk_tuc_release_fish$tag_code)), file = here::here("PTAGIS_queries", "intermediate_files", "lower_snake_tucannon_release_tags_0506_1415.txt"), col.names = FALSE, row.names = FALSE, quote = FALSE) 

# Okay, so this query only has 221 fish. It would appear that most individuals don't have the detailed tagging information

# Keep looking in the PTAGIS data
table(lwrsnk_tuc_release_fish$release_site_name)

table(lwrsnk_tuc_release_fish$hatchery_name)
table(lwrsnk_tuc_release_fish$stock_name)
