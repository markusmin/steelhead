...
adult_returns %>% 
  dplyr::select(tag_code, mark_file_name, last_time_value) -> test

test <- test[1:100,]

# test <- seq(data.frame(lat = seq(1, 10), long = seq(1,10), r.sq = seq(1,100)))

pivot_wider(test, names_from = mark_file_name, values_from = last_time_value) %>% 
  column_to_rownames("tag_code") %>% 
  as.matrix()
...
# PTAGIS workflow - single tributary
