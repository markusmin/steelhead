...
- 1 Attributes: Standard + Release Site Basin, Release Site Subbasin, mark site basin, mark site subbasin
...
# Sort into run years
run_year <- c("05/06", "06/07", "07/08", "08/09", "09/10", "10/11", "11/12", "12/13", "13/14", "14/15", "15/16", "16/17", "17/18", "18/19", "19/20", "20/21")
run_year_start <- seq(ymd("2005-06-01"), ymd("2020-06-01"), by = "years")
run_year_end <- seq(ymd("2006-05-31"), ymd("2021-05-31"), by = "years")

run_year_df <- data.frame(run_year, run_year_start, run_year_end)

adult_returns %>% 
  mutate(run_year = subset(run_year_df, run_year_start < time & run_year_end > time)$run_year) -> adult_returns
```

### Inspect distributions by run years and origins
```{r}
# basin name is not specific enough
origin_table <- as.data.frame(table(adult_returns$release_site_basin_name))
origin_table <- as.data.frame(table(adult_returns$release_site_subbasin_name))
origin_table
```
