...
annual_det_eff <- function(tributary_name, river_mouth_site, 
  ...
  trib_rm %>% 
    ...
    mutate(river_mouth_time = ymd_hms(river_mouth_time)) %>% 
    mutate(upstream_time = ymd_hms(upstream_time)) %>%
    mutate(year = ifelse(is.na(river_mouth_time), year(upstream_time),
                         year(river_mouth_time))) %>% 
    subset(!(is.na(upstream_time))) %>% # we're only looking at fish that we saw at the upstream arrays
    mutate(missed_det = ifelse(is.na(river_mouth_time), "yes", "no"))  %>% 
  ...
  trib_det_times %>% 
    ...
    mutate(det_eff = (total - n)/total) %>% 
    mutate(det_eff_SD = sqrt(det_eff * (1 - det_eff)/total)) %>% 
    # here we are using a normal approximation - will get us into trouble with small sample sizes by year, 
    # and especially as we get close to 0 or 1 (which we often do)
    mutate(det_eff_lower95 = det_eff -  1.96*det_eff_SD) %>%
    mutate(det_eff_upper95 = det_eff + 1.96*det_eff_SD) %>% 
...
plot_annual_det_eff <- function(trib_det_eff_df, site_name){
  ...
  mutate(year = as.numeric(year)) -> trib_det_eff_annual
...
trib_det_times %>% 
  mutate(river_mouth_month = month(river_mouth_time)) %>% 
  mutate(upstream_month = month(upstream_time)) %>% 
  ...
  mutate(site = gsub("_month", "", site)) -> trib_det_month
...
usgs_data_plot <- function(variables, tributary_name, usgs_file_name){
  ...
  usgs %>% 
    ...
    mutate(year = year(date_time))-> usgs
  
  # summarise into monthly averages
  usgs %>% 
    group_by(year,month) %>% 
    dplyr::summarise(mean_discharge_cfs = mean(discharge_cfs, na.rm = TRUE)) %>% 
    mutate(year_month = ymd(paste(year, month,"01",sep="-")))-> monthly_discharge
  
  # calculate annual averages
  usgs %>% 
    group_by(year) %>% 
    dplyr::summarise(mean_discharge_cfs = mean(discharge_cfs, na.rm = TRUE)) %>% 
    mutate(year_month = ymd(paste(year,"01-01",sep="-")))-> annual_discharge
...
discharge_plot <- ggplot(monthly_discharge, aes(x = year_month, y = mean_discharge_cfs)) +
  geom_line() +
  geom_line(data = annual_discharge, aes(x = year_month, y = mean_discharge_cfs), 
             color = "#1f78b4") +
...
return(discharge_plot)
  ...
  usgs %>% 
    ...
    mutate(year_month = ymd(paste(year, month,"01",sep="-")))-> monthly_gage_height
  ...
  usgs %>% 
    group_by(year) %>% 
    dplyr::summarise(mean_gage_height_ft = mean(gage_height_feet, na.rm = TRUE)) %>% 
    mutate(year_month = ymd(paste(year,"01-01",sep="-")))-> annual_gage_height
...
gage_height_plot <- ggplot(monthly_gage_height, aes(x = year_month, y = mean_gage_height_ft)) +
  geom_line() +
  geom_line(data = annual_gage_height, aes(x = year_month, y = mean_gage_height_ft),
             color = "#1f78b4") +
...
return(list(discharge_plot, gage_height_plot, gage_height_v_discharge_plot))
  }
...
entiat_river_usgs_headers = read.table(here::here("tributary_data", "entiat_river_12452990.txt"), skip = 30, header = F, nrows = 1, as.is = T, sep = "\t")
entiat_river_usgs <- read.csv(here::here("tributary_data", "entiat_river_12452990.txt"), skip = 32, header = F, sep = "\t")
colnames(entiat_river_usgs) <- entiat_river_usgs_headers

# change some column names
colnames(entiat_river_usgs) <- c("agency", "site_no", "date_time", "discharge_cfs", "discharge_cfs_approved", "gage_height_feet", "gage_height_feet_approved")
# drop most columns
entiat_river_usgs %>% 
  dplyr::select("date_time", "discharge_cfs", "gage_height_feet") %>% 
  mutate(date_time = ymd(date_time)) %>% 
  mutate(month = month(date_time)) %>% 
  mutate(year = year(date_time))-> entiat_river_usgs

entiat_river_usgs %>% 
  group_by(year,month) %>% 
  summarise(mean_discharge_cfs = mean(discharge_cfs, na.rm = TRUE)) %>% 
  mutate(year_month = ymd(paste(year, month,"01",sep="-")))-> entiat_river_monthly_discharge

entiat_river_usgs %>% 
  group_by(year,month) %>% 
  summarise(mean_gage_height_ft = mean(gage_height_feet, na.rm = TRUE)) %>% 
  mutate(year_month = ymd(paste(year, month,"01",sep="-")))-> entiat_river_monthly_gage_height

entiat_river_usgs %>% 
  group_by(year) %>% 
  summarise(mean_discharge_cfs = mean(discharge_cfs, na.rm = TRUE)) %>% 
  mutate(year_month = ymd(paste(year,"01-01",sep="-")))-> entiat_river_annual_discharge

entiat_river_usgs %>% 
  group_by(year) %>% 
  summarise(mean_gage_height_ft = mean(gage_height_feet, na.rm = TRUE)) %>% 
  mutate(year_month = ymd(paste(year,"01-01",sep="-")))-> entiat_river_annual_gage_height

# What months have the highest discharge/gage height?
# Based on our plots of arrival timing, most fish are arriving in March/April, but there are lots year round. Especially at
# the river mouth, it seems like a peak in late summer (9/10/11) and then in late winter (3/4/5). So honestly year round is probably fine
ggplot(entiat_river_monthly_discharge, aes(x = month, y = mean_discharge_cfs)) +
  geom_point() +
  scale_x_continuous(breaks = seq(1,12,1))
# Peak discharge is in May/June. We don't see many fish at tributary arrays during these months.

# plot all of them
ggplot(entiat_river_monthly_discharge, aes(x = year_month, y = mean_discharge_cfs)) +
  geom_line() +
  geom_line(data = entiat_river_annual_discharge, aes(x = year_month, y = mean_discharge_cfs), 
             color = "#1f78b4") +
  geom_point(data = entiat_river_annual_discharge, aes(x = year_month, y = mean_discharge_cfs), 
             color = "#1f78b4", shape = 23, size = 2, fill = "#a6cee3") +
  ggtitle("Entiat River Discharge") +
  xlab("Month/Year") +
  ylab("Mean Discharge (cfs)") +
  scale_y_continuous(lim = c(0, round_any(max(entiat_river_monthly_discharge$mean_discharge_cfs), 1000, f = ceiling)), expand = c(0,0))

ggplot(entiat_river_monthly_gage_height, aes(x = year_month, y = mean_gage_height_ft)) +
  geom_line() +
  geom_line(data = entiat_river_annual_gage_height, aes(x = year_month, y = mean_gage_height_ft),
             color = "#1f78b4") +
  geom_point(data = entiat_river_annual_gage_height, aes(x = year_month, y = mean_gage_height_ft),
             color = "#1f78b4", shape = 23, size = 2, fill = "#a6cee3") +
  ggtitle("Entiat River Stream Gage Height") +
  xlab("Month/Year") +
  ylab("Mean Gage Height (feet)") +
  scale_y_continuous(lim = c(0, round_any(max(entiat_river_monthly_gage_height$mean_gage_height_ft), 1, f = ceiling)), expand = c(0,0))


ggplot(entiat_river_usgs, aes(x = discharge_cfs, y = gage_height_feet, color = as.factor(year))) +
  geom_point() +
...
asotin_creek_usgs_headers = read.table(here::here("tributary_data", "asotin_creek_13335050.txt"), skip = 30, header = F, nrows = 1, as.is = T, sep = "\t")
asotin_creek_usgs <- read.csv(here::here("tributary_data", "asotin_creek_13335050.txt"), skip = 32, header = F, sep = "\t")
colnames(asotin_creek_usgs) <- asotin_creek_usgs_headers

# change some column names
colnames(asotin_creek_usgs) <- c("agency", "site_no", "date_time", "discharge_cfs", "discharge_cfs_approved", "gage_height_feet", "gage_height_feet_approved")
# drop most columns
asotin_creek_usgs %>% 
  dplyr::select("date_time", "discharge_cfs", "gage_height_feet") %>% 
  mutate(date_time = ymd(date_time)) %>% 
  mutate(month = month(date_time)) %>% 
  mutate(year = year(date_time))-> asotin_creek_usgs

asotin_creek_usgs %>% 
  group_by(year,month) %>% 
  summarise(mean_discharge_cfs = mean(discharge_cfs, na.rm = TRUE)) %>% 
  mutate(year_month = ymd(paste(year, month,"01",sep="-")))-> asotin_creek_monthly_discharge

asotin_creek_usgs %>% 
  group_by(year,month) %>% 
  summarise(mean_gage_height_ft = mean(gage_height_feet, na.rm = TRUE)) %>% 
  mutate(year_month = ymd(paste(year, month,"01",sep="-")))-> asotin_creek_monthly_gage_height

asotin_creek_usgs %>% 
  group_by(year) %>% 
  summarise(mean_discharge_cfs = mean(discharge_cfs, na.rm = TRUE)) -> asotin_creek_annual_discharge

asotin_creek_usgs %>% 
  group_by(year) %>% 
  summarise(mean_gage_height_ft = mean(gage_height_feet, na.rm = TRUE)) -> asotin_creek_annual_gage_height

# plot all of them
ggplot(asotin_creek_monthly_discharge, aes(x = year_month, y = mean_discharge_cfs)) +
  geom_line()

ggplot(asotin_creek_monthly_gage_height, aes(x = year_month, y = mean_gage_height_ft)) +
  geom_line()

ggplot(asotin_creek_annual_discharge, aes(x = year, y = mean_discharge_cfs)) +
  geom_line()

ggplot(asotin_creek_annual_gage_height, aes(x = year, y = mean_gage_height_ft)) +
  geom_line()

# what's the strength of the relationship between gage height and discharge?
ggplot(asotin_creek_usgs, aes(x = discharge_cfs, y = gage_height_feet)) +
  geom_point() +
  geom_smooth(formula = gage_height_feet ~ log(discharge_cfs)) +
  ggtitle("Asotin Creek")

ggplot(asotin_creek_usgs, aes(x = discharge_cfs, y = gage_height_feet, color = as.factor(year))) +
  geom_point() +
  ggtitle("Asotin Creek")
...
as.data.frame(ACM_det_eff) %>% 
  mutate(det_eff = round(det_eff, 3)) %>% 
  mutate(det_eff_lower95 = round(det_eff_lower95, 3)) %>% 
  mutate(det_eff_upper95 = round(det_eff_upper95, 3)) %>% 
...
ACM_det_eff %>% 
  ...
  mutate(time_period = ifelse(year <= 2018, "2011-2018", "2019-present"))-> ACM_deteff_gageheight
...
LWE_det_eff <- annual_det_eff(tributary_name = "Wenatchee River", river_mouth_site = "LWE - Lower Wenatchee River", 
                           river_mouth_site_start_date = "2009-10-01", river_mouth_site_end_date = "2023-01-01")
