...
entiat_river_usgs %>% 
  group_by(year) %>% 
  summarise(mean_discharge_cfs = mean(discharge_cfs, na.rm = TRUE)) -> entiat_river_annual_discharge

entiat_river_usgs %>% 
  group_by(year) %>% 
  summarise(mean_gage_height_ft = mean(gage_height_feet, na.rm = TRUE)) -> entiat_river_annual_gage_height
...
ggplot(entiat_river_annual_discharge, aes(x = year, y = mean_discharge_cfs)) +
  geom_line()

ggplot(entiat_river_annual_gage_height, aes(x = year, y = mean_gage_height_ft)) +
  geom_line()

# what's the strength of the relationship between gage height and discharge?
ggplot(entiat_river_usgs, aes(x = discharge_cfs, y = gage_height_feet)) +
  geom_point() +
  geom_smooth(formula = gage_height_feet ~ log(discharge_cfs)) +
