...
ASC %>% 
  ...
  # add in the next date
  mutate(date_time_2 = lead(date_time)) %>% 
...
UCW_ASC %>% 
  ...
  mutate(fallback = ifelse(next_state == "mainstem, RRE to WEL", 1, 0)) %>% 
  filter() -> UCW_ASC_7_6_janspill
...
UCW_ASC %>% 
  ...
  filter(state == "mainstem, upstream of WEL") %>% 
  mutate(fallback = ifelse(next_state == "mainstem, RRE to WEL", 1, 0)) %>% 
  mutate(month_1 = month(date_time)) %>% 
  mutate(month_2 = month(date_time_2)) %>% 
  mutate(apr = ifelse(month_1 > month_2,
                      ifelse(4 %in% c(seq(month_1, 12), seq(1, month_2)), 1, 0),
                ifelse(4 %in% seq(month_1, month_2), 1, 0)))  %>% 
  filter(apr == 1)  -> UCW_ASC_7_6_aprspill

         
         
table(UCW_ASC_7_6_aprspill$next_state)
# table(UCW_ASC_7_6_aprspill$natal_origin)
...
A very weakly negative relationship (but this doesn't make sense to me).
