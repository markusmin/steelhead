...
for(i in 1:n.ind){ # Loop through the detection matrices for each individual
  ...
  brear_matrix[states_mat[i,j], 9] + borigin_matrix[states_mat[i,j], 9]))))
    ...
    # In this simulation, we will only have priors for the b0_matrix. Everything else will be fixed to zero.
        for (i in 1:nmovements){
    b0_matrix[movements[i,1], movements[i,2]] ~ dnorm(0,0.001)
        }
    
        for (i in 1:nmovements){
    bflow_matrix[movements[i,1], movements[i,2]] <- 0
        }
    
        for (i in 1:nmovements){
    btemp_matrix[movements[i,1], movements[i,2]] <- 0
        }
    
        for (i in 1:nmovements){
    brear_matrix[movements[i,1], movements[i,2]] <- 0
        }
    
        for (i in 1:nmovements){
    borigin_matrix[movements[i,1], movements[i,2]] <- 0
        }
...
##### JAGS model - CATEGORICAL COVARIATES (origin + rear) #####
cat("
model {
...
}", fill=TRUE, file=here::here("simulation", "sim_model_cov_categorical.txt")) 
...
##### JAGS model - ALL 4 COVARIATES (origin + rear + temp + flow) #####
cat("
model {
...
}", fill=TRUE, file=here::here("simulation", "sim_model_all4cov.txt")) 
...
saveRDS(out.jags, here::here("simulation", "JAGS_nocov_3chains_15kiter_5kburnin_4.rds"))
