...
### load data
```{r load_data}
temp_yhat <- read.csv(here::here("covariate_data", "for_model", "temp", "temp_yhat.csv"))

temp_yhat$date <- ymd(temp_yhat$date)

# extract each dam
BON_temp_ts <- dplyr::select(temp_yhat, date, BON)
MCN_temp_ts <- dplyr::select(temp_yhat, date, MCN)
PRA_temp_ts <- dplyr::select(temp_yhat, date, PRA)
RIS_temp_ts <- dplyr::select(temp_yhat, date, RIS)
RRE_temp_ts <- dplyr::select(temp_yhat, date, RRE)
WEL_temp_ts <- dplyr::select(temp_yhat, date, WEL)
ICH_temp_ts <- dplyr::select(temp_yhat, date, ICH)
LGR_temp_ts <- dplyr::select(temp_yhat, date, LGR)
...
WEL_quick_window_temp <- window_temp(temp_data = WEL_temp_ts, median_time = round(WEL_mix$mu[1]))

# Wells Dam, for upstream of WEL reach - slow fish
WEL_slow_window_temp <- window_temp(temp_data = WEL_temp_ts, median_time = round(WEL_mix$mu[2]))

# Lower Granite Dam, for upstream of LGR reach - quick fish
LGR_quick_window_temp <- window_temp(temp_data = LGR_temp_ts, median_time = round(LGR_mix$mu[1]))

# Lower Granite Dam, for upstream of LGR reach - slow fish
LGR_slow_window_temp <- window_temp(temp_data = LGR_temp_ts, median_time = round(LGR_mix$mu[2]))
