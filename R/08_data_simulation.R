...
# Here as a list
saveRDS(det_hist_sim, here::here("simulation", "sim_600_nocov_list.rds"))
...
saveRDS(det_hist_sim_array, here::here("simulation", "sim_600_nocov_array.rds"))

# Export the transition dates
saveRDS(transition_date_sim, here::here("simulation", "dates_600_nocov.rds"))
# As an array
...
saveRDS(transition_date_sim_numeric, here::here("simulation", "dates_600_nocov_matrix.rds"))


##### 1200 fish #####

fish_sim_cat_data <- data.frame(tag_code = seq(1, 1200, 1),
...
# Export the detection history
# Here as a list
saveRDS(det_hist_sim, here::here("simulation", "sim_1200_nocov_list.rds"))
...
max_obs <- max(n.obs)
det_hist_sim_array <- movement_array[,1:max_obs,]
saveRDS(det_hist_sim_array, here::here("simulation", "sim_1200_nocov_array.rds"))

# Export the transition dates
saveRDS(transition_date_sim, here::here("simulation", "dates_1200_nocov.rds"))
# As an array
transition_date_sim_matrix <- transition_date[,1:max_obs]
...
saveRDS(transition_date_sim_numeric, here::here("simulation", "dates_1200_nocov_matrix.rds"))
...
# Export the detection history
# Here as a list
saveRDS(det_hist_sim, here::here("simulation", "sim_3000_nocov_list.rds"))
...
max_obs <- max(n.obs)
det_hist_sim_array <- movement_array[,1:max_obs,]
saveRDS(det_hist_sim_array, here::here("simulation", "sim_3000_nocov_array.rds"))

# Export the transition dates
saveRDS(transition_date_sim, here::here("simulation", "dates_3000_nocov.rds"))
# As an array
transition_date_sim_matrix <- transition_date[,1:max_obs]
...
saveRDS(transition_date_sim_numeric, here::here("simulation", "dates_3000_nocov_matrix.rds"))



##### SIMULATION 2: COVARIATES #####
# Create a matrix to store the dates in each state; make ncol 100 to accommodate 100 possible state visits
state_date <- matrix(nrow = nfish, ncol = 100)
...
# First, let's see what happens if we increase all b0s at the same rate, and have no covariates
b0_vals <- seq(-100, 100, 1)

# Create a matrix to store the transition probabilities
probs_df <- as.data.frame(matrix(ncol = 6, nrow = length(b0_vals)))
...
for (i in 1:length(b0_vals)){
  ...
  probs_df[i,1] <- phi_BM_MB/(1 + phi_BM_MB + phi_BM_MIP + phi_BM_DES + phi_BM_JDR)
  probs_df[i,2] <- phi_BM_MIP/(1 + phi_BM_MB + phi_BM_MIP + phi_BM_DES + phi_BM_JDR)
  probs_df[i,3] <- phi_BM_DES/(1 + phi_BM_MB + phi_BM_MIP + phi_BM_DES + phi_BM_JDR)
  probs_df[i,4] <- phi_BM_JDR/(1 + phi_BM_MB + phi_BM_MIP + phi_BM_DES + phi_BM_JDR)
  probs_df[i,5] <- 1 - sum(probs_df[i,1:4])
  probs_df[i,6] <- b0
}

# pivot longer
probs_df %>% 
  pivot_longer(., cols = colnames(probs_df[,1:5])) %>%
  dplyr::rename(next_state = name, prob = value, b0 = V6) %>% 
  mutate(next_state = ifelse(next_state == "V5", "loss", next_state)) %>%
  mutate(next_state = gsub("V", "", next_state))-> probs_df_long

ggplot(probs_df_long, aes(x = b0, y = prob, color = next_state)) +
  geom_point()
