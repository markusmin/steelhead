...
3. If neither is available, use the relationship between temperature at the current dam and temperature at a nearby dam to interpolate (we will determine what that relationship is in this script)
4. If no dam has available temperature data for the date in question, use a linear interpolation between the last two temperature data points.
...
### filtering function
Two steps:
1. For total nonsense runs of values (RIS, RRE, and WEL) - manually select and drop those days
2. Drop outliers - see chunk below
```{r filter_Wells}
# WEL, forebay
WEL_temp_f_long %>% 
  mutate(date = ymd(date)) %>% 
