...
# FOR TESTING: setwd
setwd("/Users/markusmin/Documents/CBR/steelhead/stan_simulation/04_temp")
...
sim_1200_hist_list <- readRDS("sim_1200_cov_temp_hist_list.rds")
sim_1200_dates_list <- readRDS("sim_1200_cov_temp_dates_list.rds")
fish_sim_cat_data_1200 <- as.data.frame(as.matrix(read.csv("origin_rear_1200.csv")))


# Create a list to store JAGS objects
stan_1200_temp_list <- list()
# Loop it
...
# for (z in 1:1){
  ...
  mod <- cmdstan_model("04_stan_sim_int_temp.stan", compile = FALSE)
  ...
  stan_1200_temp_list[[z]] <- fit$summary(variables = c("b0_matrix_2_1", 
                                                   "b0_matrix_1_2",
                                                   "b0_matrix_3_2",
                                                   "b0_matrix_6_2",
                                                   "b0_matrix_7_2",
...
simulation_plots_temp <- function(stan_list){
...
sim1200_temp_plots <- simulation_plots_temp(stan_list = stan_1200_temp_list)
ggsave(here::here("stan_simulation", "04_temp", "stan_temp_summary_plot_b0.png"), height = 6, width = 10, sim1200_temp_plots[[1]])
ggsave(here::here("stan_simulation", "04_temp", "stan_temp_summary_plot_btemp.png"), height = 6, width = 10, sim1200_temp_plots[[2]])
