# UPPER COLUMBIA - 02_stan_actual_int_origin - for mox!
...
# FOR TESTING: setwd
# setwd("/Users/markusmin/Documents/CBR/steelhead/stan_actual/ESU_models/upper_columbia/")
...
# 11: John Day River
transition_matrix["John Day River", "loss"] <- 0
transition_matrix["John Day River", "mainstem, BON to MCN"] <- 0

# 13: Fifteenmile Creek
transition_matrix["Fifteenmile Creek", "loss"] <- 0
transition_matrix["Fifteenmile Creek", "mainstem, BON to MCN"] <- 0

# 14: Umatilla River
transition_matrix["Umatilla River", "loss"] <- 0
transition_matrix["Umatilla River", "mainstem, BON to MCN"] <- 0

# 15: Yakima River
transition_matrix["Yakima River", "loss"] <- 0
transition_matrix["Yakima River", "mainstem, MCN to ICH or PRA"] <- 0

# 16: Walla Walla River
transition_matrix["Walla Walla River", "loss"] <- 0
transition_matrix["Walla Walla River", "mainstem, MCN to ICH or PRA"] <- 0

# 21: Tucannon River
transition_matrix["Tucannon River", "loss"] <- 0
transition_matrix["Tucannon River", "mainstem, ICH to LGR"] <- 0

# 22: Asotin Creek
transition_matrix["Asotin Creek", "loss"] <- 0
transition_matrix["Asotin Creek", "mainstem, upstream of LGR"] <- 0

# 24: Salmon River
transition_matrix["Salmon River", "loss"] <- 0
transition_matrix["Salmon River", "mainstem, upstream of LGR"] <- 0

# 25: Grande Ronde River
transition_matrix["Grande Ronde River", "loss"] <- 0
transition_matrix["Grande Ronde River", "mainstem, upstream of LGR"] <- 0

# 26: Imnaha River
transition_matrix["Imnaha River", "loss"] <- 0
transition_matrix["Imnaha River", "mainstem, upstream of LGR"] <- 0

# 27: BON to MCN other tributaries
transition_matrix["BON to MCN other tributaries", "loss"] <- 0
transition_matrix["BON to MCN other tributaries", "mainstem, BON to MCN"] <- 0
...
upper_columbia_movements <- subset(movements_df, row %in% c(4,5,6,7,17,18,19,20,28) |
                                     col %in% c(4,5,6,7,17,18,19,20,28))

non_upper_columbia_movements  <- subset(movements_df, !(row %in% c(4,5,6,7,17,18,19,20,28)) &
                                          !(col %in% c(4,5,6,7,17,18,19,20,28)))
...
# Here, that means we limit the effect to states 4-7, 17-20, and 28
for (i in 1:3){
...
for (i in 1:3){
  for (j in 1:nrow(upper_columbia_movements)){
    cat("borigin", i, "_matrix[", upper_columbia_movements$row[j], ",", upper_columbia_movements$col[j], "]", " = ", "borigin", i, "_matrix_", upper_columbia_movements$row[j], "_", upper_columbia_movements$col[j], ";", "\n", sep = "")
  }
  cat ("\n")
}
...
for (i in 1:3){
  for (j in 1:nrow(non_upper_columbia_movements)){
    cat("borigin", i, "_matrix[", non_upper_columbia_movements$row[j], ",", non_upper_columbia_movements$col[j], "]", " = ", 0, ";", "\n", sep = "")
  }
  cat ("\n")
}
...
n.ind <- dim(state_data)[3]
  ...
  mod <- cmdstan_model("parallel_upper_columbia_02_stan_actual_int_origin.stan", compile = FALSE)
...
fit$save_object(file = "seed101_200_iter_parallel_upper_columbia_stan_actual_int_origin_stan_fit.rds")
...
states_complete %>% 
  ...
  mutate(transition = ifelse(tag_code_2 == lag(tag_code_2), paste0(lag(state), " - ", state))) %>% 
  mutate(transition_numeric = ifelse(tag_code_2 == lag(tag_code_2), paste0(lag(index), " - ", index))) -> state_transitions
