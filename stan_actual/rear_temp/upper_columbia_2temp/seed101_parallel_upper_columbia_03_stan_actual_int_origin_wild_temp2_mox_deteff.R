# UPPER COLUMBIA - 03_stan_actual_int_origin_wild_temp - for mox!

# in this version of the model, we now have tributary detection efficiencies, an intercept, and a temperature effect
...
# FOR TESTING: setwd
# setwd("/Users/markusmin/Documents/CBR/steelhead/stan_actual/rear_temp/upper_columbia_2temp/")
...
# create a temp2 matrix - this is only for the most upstream states, but could be modified in the future
# note also here that it'll only be relevant for upstream of WEL in this example, because 
# we're not going to be able to estimate a second temperature effect for outside the DPS (far too little data)
btempxorigin_matrix_names %>%
  filter(row %in% c(7)) -> btemp2xorigin_matrix_names
...
# print temp2 x origin 
for (i in 1:3){
  for (j in 1:nrow(btemp2xorigin_matrix_names)){
    
    # Movements from mainstem to river mouth: print two versions
    if (btemp2xorigin_matrix_names$mainstem_river_mouth_movement[j] == 1){
      cat("real ", "btemp2xorigin", i, "_matrix_", btemp2xorigin_matrix_names$row[j], "_", btemp2xorigin_matrix_names$col[j], "_DE", ";", "\n", sep = "")
      cat("real ", "btemp2xorigin", i, "_matrix_", btemp2xorigin_matrix_names$row[j], "_", btemp2xorigin_matrix_names$col[j], "_NDE", ";", "\n", sep = "")
    }
    
    # If it's a within tributary movement - we don't want it
    else if (btemp2xorigin_matrix_names$within_trib_movement[j] == 1){
      # do nothing!
    }
    # If it's a movement from mainstem to upstream - we also don't want it
    else if (btemp2xorigin_matrix_names$mainstem_upstream_movement[j] == 1){
      # do nothing!
    } 
    # If we move from the river mouth state to the mainstem, we want a parameter - this is captured in the below
    # BUT - if it's a movement from the upstream state to the mainstem, we don't want a parameter.
    # these transitions will get the same parameters as the river mouth to mainstem
    else if (btemp2xorigin_matrix_names$upstream_mainstem_movement[j] == 1){
      # do nothing!
    } else {
      # Finally - if it's any other movement, just treat it like normal!
      # If it's not, print just the one version
      cat("real ", "btemp2xorigin", i, "_matrix_", btemp2xorigin_matrix_names$row[j], "_", btemp2xorigin_matrix_names$col[j], ";", "\n", sep = "")
    }
  }
}
...
# Write out the tempxorigin matrix - DE and NDE
for (i in 1:3){
  for (j in 1:nrow(upper_columbia_tempxorigin)){
    # Movements from mainstem to river mouth: store the two different versions
    if (upper_columbia_tempxorigin$mainstem_river_mouth_movement[j] == 1){
...
# Write out the temp2xorigin matrix - DE and NDE
for (i in 1:3){
  for (j in 1:nrow(btemp2xorigin_matrix_names)){
    # Movements from mainstem to river mouth: store the two different versions
    if (btemp2xorigin_matrix_names$mainstem_river_mouth_movement[j] == 1){
      cat("btemp2xorigin", i, "_matrix_DE[", btemp2xorigin_matrix_names$row[j], ",", btemp2xorigin_matrix_names$col[j], "]", " = ", "btemp2xorigin", i, "_matrix_", btemp2xorigin_matrix_names$row[j], "_", btemp2xorigin_matrix_names$col[j], "_DE", ";", "\n", sep = "")
      cat("btemp2xorigin", i, "_matrix_NDE[", btemp2xorigin_matrix_names$row[j], ",", btemp2xorigin_matrix_names$col[j], "]", " = ", "btemp2xorigin", i, "_matrix_", btemp2xorigin_matrix_names$row[j], "_", btemp2xorigin_matrix_names$col[j], "_NDE", ";", "\n", sep = "")
    ...
    # For DE - we don't care about this, because we're removing the upstream states
      ...
      cat("btemp2xorigin", i, "_matrix_NDE[", btemp2xorigin_matrix_names$row[j], ",", btemp2xorigin_matrix_names$col[j], "]", " = ", "btemp2xorigin", i, "_matrix_", btemp2xorigin_matrix_names$row[j-1], "_", btemp2xorigin_matrix_names$col[j-1], "_NDE", ";", "\n", sep = "")
      
    }
    # If it's a movement from the upstream state back to the mainstem, give it the same parameter as movement from the river mouth to the upstream
    # And note - that these are not DE or NDE parameters
    else if (btemp2xorigin_matrix_names$upstream_mainstem_movement[j] == 1){
      cat("btemp2xorigin", i, "_matrix_DE[", btemp2xorigin_matrix_names$row[j], ",", btemp2xorigin_matrix_names$col[j], "]", " = ", "btemp2xorigin", i, "_matrix_", btemp2xorigin_matrix_names$row[j-1], "_", btemp2xorigin_matrix_names$col[j],  ";", "\n", sep = "")
      cat("btemp2xorigin", i, "_matrix_NDE[", btemp2xorigin_matrix_names$row[j], ",", btemp2xorigin_matrix_names$col[j], "]", " = ", "btemp2xorigin", i, "_matrix_", btemp2xorigin_matrix_names$row[j-1], "_", btemp2xorigin_matrix_names$col[j], ";", "\n", sep = "")
      # If it's not, store the same parameter in both matrices
    } else {
      cat("btemp2xorigin", i, "_matrix_DE[", btemp2xorigin_matrix_names$row[j], ",", btemp2xorigin_matrix_names$col[j], "]", " = ", "btemp2xorigin", i, "_matrix_", btemp2xorigin_matrix_names$row[j], "_", btemp2xorigin_matrix_names$col[j], ";", "\n", sep = "")
      cat("btemp2xorigin", i, "_matrix_NDE[", btemp2xorigin_matrix_names$row[j], ",", btemp2xorigin_matrix_names$col[j], "]", " = ", "btemp2xorigin", i, "_matrix_", btemp2xorigin_matrix_names$row[j], "_", btemp2xorigin_matrix_names$col[j],  ";", "\n", sep = "")
    }
...
# Priors for temp2 xorigin
for (i in 1:3){
  for (j in 1:nrow(btemp2xorigin_matrix_names)){
    
    # Movements from mainstem to river mouth: print two versions
    if (btemp2xorigin_matrix_names$mainstem_river_mouth_movement[j] == 1){
      cat("btemp2xorigin", i, "_matrix_", btemp2xorigin_matrix_names$row[j], "_", btemp2xorigin_matrix_names$col[j], "_DE", " ~ normal(0,10);", "\n", sep = "")
      cat("btemp2xorigin", i, "_matrix_", btemp2xorigin_matrix_names$row[j], "_", btemp2xorigin_matrix_names$col[j], "_NDE", " ~ normal(0,10);", "\n", sep = "")
    }
    ...
    # these transitions will get the same parameters as the river mouth to mainstem
      ...
      # Finally - if it's any other movement, just treat it like normal!
      # If it's not, print just the one version
      cat("btemp2xorigin", i, "_matrix_", btemp2xorigin_matrix_names$row[j], "_", btemp2xorigin_matrix_names$col[j], " ~ normal(0,10);", "\n", sep = "")
    }
...
for (i in 1:nrow(states_complete)){
  transition_date_matrix[states_complete[i,"tag_code_number", drop = TRUE], states_complete[i, "order", drop = TRUE]] <- as.character(date(states_complete[i, "date_time", drop = TRUE]))
}
...
n.ind <- dim(state_data)[3]
  ...
  mod <- cmdstan_model("parallel_upper_columbia_03_stan_actual_int_origin_wild_temp2_deteff.stan", compile = FALSE)
