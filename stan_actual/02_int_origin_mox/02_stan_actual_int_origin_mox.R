# 02_stan_actual_int_origin - for mox!

# This script will fit an intercept + origin model using stan to the actual dataset

# FOR TESTING: setwd
setwd("/Users/markusmin/Documents/CBR/steelhead/stan_actual/02_int_origin_mox/")
...
# Paste all 16 origin parameters (for 17 origins, with one as reference)
for (i in 1:16){
  for (j in 1:nrow(movements)){
    cat("real ", "borigin", i, "_matrix_", movements_df$row[j], "_", movements_df$col[j], ";", "\n", sep = "")
  }
  cat ("\n")
}
...
for (i in 1:16){
  for (j in 1:nrow(movements)){
    cat("borigin", i, "_matrix[", movements_df$row[j], ",", movements_df$col[j], "]", " = ", "borigin", i, "_matrix_", movements_df$row[j], "_", movements_df$col[j], ";", "\n", sep = "")
  }
  cat ("\n")
}
...
for (i in 1:16){
  for (j in 1:nrow(movements)){
    cat("borigin", i, "_matrix_", movements_df$row[j], "_", movements_df$col[j], " ~ normal(0,10);", "\n", sep = "")
  }
  cat ("\n")
}
...
states_complete <- read.csv("adults_states_complete.csv", row.names = 1)
...
saveRDS(state_data, "state_data.csv")
...
write.csv(origin_rear_actual,"origin_rear_actual.csv")
...
n.ind <- dim(state_data)[3]
  ...
  mod <- cmdstan_model("02_stan_actual_int_origin.stan", compile = FALSE)
...
saveRDS(fit, "stan_actual_int_origin_stan_fit.rds")
