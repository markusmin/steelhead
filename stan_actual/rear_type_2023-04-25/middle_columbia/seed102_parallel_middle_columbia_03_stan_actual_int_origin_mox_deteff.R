# Middle Columbia - 03_stan_actual_int_origin - for mox!
...
# FOR TESTING: setwd
# setwd("/Users/markusmin/Documents/CBR/steelhead/stan_actual/deteff_ESU_models/middle_columbia/")
...
# 3) "Yakima River Upstream"      
# 4) "Walla Walla River Upstream" 
# 5) "Wenatchee River Upstream"  
# 6) "Entiat River Upstream"      
# 7) "Entiat River Mouth"      
# 8) ""Okanogan River Upstream"        
# 9) "Methow River Upstream"          
# 10) "Tucannon River Upstream"       
# 11) "Imnaha River Upstream"          
# 12) "Upstream WEL other tributaries"
...
transition_matrix["Entiat River Mouth", ] <- 0
transition_matrix["Entiat River Upstream", ] <- 0
transition_matrix["Okanogan River Upstream", ] <- 0
transition_matrix["Methow River Upstream", ] <- 0
transition_matrix["Tucannon River Upstream", ] <- 0
transition_matrix["Imnaha River Upstream", ] <- 0
transition_matrix["Upstream WEL other tributaries", ] <- 0
...
# middle_columbia_movements <- subset(movements_df, row %in% c(8,9,21,22,23,24,25,26) |
...
middle_columbia_movements %>% 
  arrange(row, col) -> middle_columbia_movements
...
for (i in 1:5){
  for (j in 1:nrow(middle_columbia_movements)){
    ...
    # Movements from mainstem to river mouth: print two versions
    if (middle_columbia_movements$mainstem_river_mouth_movement[j] == 1){
      cat("real ", "borigin", i, "_matrix_", middle_columbia_movements$row[j], "_", middle_columbia_movements$col[j], "_DE", ";", "\n", sep = "")
      cat("real ", "borigin", i, "_matrix_", middle_columbia_movements$row[j], "_", middle_columbia_movements$col[j], "_NDE", ";", "\n", sep = "")
    }
    
    # If it's a within tributary movement - we don't want it
    else if (middle_columbia_movements$within_trib_movement[j] == 1){
      # do nothing!
    }
    # If it's a movement from mainstem to upstream - we also don't want it
    else if (middle_columbia_movements$mainstem_upstream_movement[j] == 1){
      # do nothing!
    } 
    # If we move from the river mouth state to the mainstem, we want a parameter - this is captured in the below
    # BUT - if it's a movement from the upstream state to the mainstem, we don't want a parameter.
    # these transitions will get the same parameters as the river mouth to mainstem
    else if (middle_columbia_movements$upstream_mainstem_movement[j] == 1){
      # do nothing!
    } else {
      # Finally - if it's any other movement, just treat it like normal!
      # If it's not, print just the one version
...
for (i in 1:5){
  for (j in 1:nrow(middle_columbia_movements)){
    # Movements from mainstem to river mouth: store the two different versions
    if (middle_columbia_movements$mainstem_river_mouth_movement[j] == 1){
      cat("borigin", i, "_matrix_DE[", middle_columbia_movements$row[j], ",", middle_columbia_movements$col[j], "]", " = ", "borigin", i, "_matrix_", middle_columbia_movements$row[j], "_", middle_columbia_movements$col[j], "_DE", ";", "\n", sep = "")
      cat("borigin", i, "_matrix_NDE[", middle_columbia_movements$row[j], ",", middle_columbia_movements$col[j], "]", " = ", "borigin", i, "_matrix_", middle_columbia_movements$row[j], "_", middle_columbia_movements$col[j], "_NDE", ";", "\n", sep = "")
    ...
    # For DE - we don't care about this, because we're removing the upstream states
      ...
      # cat("borigin", i, "_matrix_DE[", middle_columbia_movements$row[j], ",", middle_columbia_movements$col[j], "]", " = ", "borigin", i, "_matrix_", middle_columbia_movements$row[j-1], "_", middle_columbia_movements$col[j-1], "_DE", ";", "\n", sep = "")
      cat("borigin", i, "_matrix_NDE[", middle_columbia_movements$row[j], ",", middle_columbia_movements$col[j], "]", " = ", "borigin", i, "_matrix_", middle_columbia_movements$row[j-1], "_", middle_columbia_movements$col[j-1], "_NDE", ";", "\n", sep = "")
      
    }
    # If it's a movement from the upstream state back to the mainstem, give it the same parameter as movement from the river mouth to the upstream
    # And note - that these are not DE or NDE parameters
    else if (middle_columbia_movements$upstream_mainstem_movement[j] == 1){
      cat("borigin", i, "_matrix_DE[", middle_columbia_movements$row[j], ",", middle_columbia_movements$col[j], "]", " = ", "borigin", i, "_matrix_", middle_columbia_movements$row[j-1], "_", middle_columbia_movements$col[j],  ";", "\n", sep = "")
      cat("borigin", i, "_matrix_NDE[", middle_columbia_movements$row[j], ",", middle_columbia_movements$col[j], "]", " = ", "borigin", i, "_matrix_", middle_columbia_movements$row[j-1], "_", middle_columbia_movements$col[j], ";", "\n", sep = "")
      # If it's not, store the same parameter in both matrices
    } else {
      cat("borigin", i, "_matrix_DE[", middle_columbia_movements$row[j], ",", middle_columbia_movements$col[j], "]", " = ", "borigin", i, "_matrix_", middle_columbia_movements$row[j], "_", middle_columbia_movements$col[j], ";", "\n", sep = "")
      cat("borigin", i, "_matrix_NDE[", middle_columbia_movements$row[j], ",", middle_columbia_movements$col[j], "]", " = ", "borigin", i, "_matrix_", middle_columbia_movements$row[j], "_", middle_columbia_movements$col[j],  ";", "\n", sep = "")
    }
...
for (i in 1:5){
  for (j in 1:nrow(middle_columbia_movements)){
    ...
    # Movements from mainstem to river mouth: print two versions
    if (middle_columbia_movements$mainstem_river_mouth_movement[j] == 1){
      cat("borigin", i, "_matrix_", middle_columbia_movements$row[j], "_", middle_columbia_movements$col[j], "_DE", " ~ normal(0,10);", "\n", sep = "")
      cat("borigin", i, "_matrix_", middle_columbia_movements$row[j], "_", middle_columbia_movements$col[j], "_NDE", " ~ normal(0,10);", "\n", sep = "")
    }
...
n.ind <- dim(state_data)[3]
  ...
  mod <- cmdstan_model("parallel_middle_columbia_03_stan_actual_int_origin_deteff.stan", compile = FALSE)
...
# saveRDS(fit, "100iter_parallel_middle_columbia_stan_actual_int_origin_stan_fit.rds")
fit$save_object(file = "seed102_500iter_parallel_middle_columbia_stan_actual_int_origin_stan_fit.rds")
