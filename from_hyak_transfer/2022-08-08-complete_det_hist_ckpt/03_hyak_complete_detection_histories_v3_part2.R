...
# FOR CKPT - split the det hist in half, by fish
det_hist_tag_codes <- unique(det_hist$tag_code)
first_half_tag_codes <- det_hist_tag_codes[1:round(length(det_hist_tag_codes)/2,0)]
# take the first half of the fish
# det_hist <- subset(det_hist, tag_code %in% first_half_tag_codes)
# take the second half of the fish
det_hist <- subset(det_hist, !(tag_code %in% first_half_tag_codes))
